                            ...
                            <div class="col-lg-6 mb-3">
                                ...
                                <select class="form-select @error('role') is-invalid @enderror" id="role"
                                    ...
                                    <option value="author" {{ old('role', $user->role) == 'author' ? 'selected' : '' }}>
                                        Author</option>
                                    <option value="users" {{ old('role', $user->role) == 'users' ? 'selected' : '' }}>users
                                    </option> {{-- Tambahkan opsi 'users' --}}
                                </select>
                                @error('role')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            const usersProvinceId =
                "{{ old('province', $user->province) }}"; // Asumsi Anda menyimpan ID provinsi di DB
            const usersCityId = "{{ old('city', $user->city) }}"; // Asumsi Anda menyimpan ID kota di DB
            ...
            function loadProvinces() {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        // Set nilai provinsi jika ada (dari old() atau dari users data)
                        if (usersProvinceId) {
                            $('#province').val(usersProvinceId);
                            // Trigger change untuk memuat kota jika provinsi sudah terpilih
                            if (usersCityId) {
                                loadCities(usersProvinceId, usersCityId);
                            } else {
                                loadCities(
                                    usersProvinceId
                                    ); // Load cities without pre-selecting if no usersCityId
                            }
                        }
                    },
                ...
                if (provinceId) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            // Set nilai kota jika ada (dari old() atau dari users data)
                            if (selectedCityId) {
